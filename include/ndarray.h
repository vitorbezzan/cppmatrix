...
public:
  ...
  template <uint64_t ndim> explicit NDArray(const uint64_t (&shape)[ndim]) {
  ...
  inline uint64_t ndim() const { return this->_ndim; }

  inline uint64_t *shape() const { return this->_shape; }

  inline T *data() const { return this->_data; }

  template <uint64_t ndim> T &operator()(uint64_t (&index)[ndim]) {
    uint64_t _index = 0;
  ...
  template <typename U> inline bool check_sizes(const NDArray<U> &right) const {
  ...
  NDArray<T> &operator+=(const NDArray<T> &right) {
    if (this->check_sizes(right)) {
      std::transform(this->_data, this->_data + this->N(), right._data,
                     this->_data, std::plus<T>());
      return *this;
    }
  ...
  NDArray<T> &operator*=(const T &right) {
    std::transform(
  ...
  NDArray<T> operator*(const T &right) { return NDArray<T>(*this) *= right; }
